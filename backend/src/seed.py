# 本地演示种子数据（全部本地数据，禁止第三方 API）。
# 场景设计：
# - 设备 1（灭火器）：CRITICAL 隐患 OPEN，已按“同设备同检查项”合并过 2 次不合格结果，设备被锁定为 ABNORMAL
# - 设备 2（消火栓）：HIGH 隐患已由维保提交整改，处于 RECHECK，等待物业主管确认恢复
# - 设备 3/4/5：NORMAL，无有效隐患
seed = {
  "building": [
    {
      "id": 1,
